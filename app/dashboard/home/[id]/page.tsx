  ...
  return (
    <div className="min-h-screen pb-20" style={{ background: 'var(--bg-secondary)' }}>
      ...
      <div className="max-w-[1200px] mx-auto px-6 pt-10 pb-6">
        <Link href="/dashboard/home" className="inline-flex items-center gap-2 text-[14px] font-[700] hover:underline" style={{ color: 'var(--cue-green)' }}>
      ...
      <div className="max-w-[1200px] mx-auto px-6 space-y-8">
        ...
        {/* HERO CARD */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-[24px] p-10 flex flex-col md:flex-row justify-between items-center md:items-start gap-10" style={{ boxShadow: 'var(--shadow-float)', border: '1px solid var(--border-light)' }}>

          <div className="flex items-center gap-6">
            <div className="w-[84px] h-[84px] rounded-full flex items-center justify-center text-[32px] font-[800] text-white shadow-md shrink-0" style={{ background: 'var(--gradient-green)' }}>
              {data.candidates?.name?.charAt(0).toUpperCase()}
            </div>
            <div>
              <h1 className="text-[32px] font-[800] text-[var(--text-primary)] mb-1 leading-tight">{data.candidates?.name}</h1>
              <div className="text-[15px] font-[500] text-[var(--text-muted)] flex flex-wrap items-center gap-3">
                <span>{data.candidates?.email}</span>
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--border-medium)]" />
                <span>{new Date(data.completed_at).toLocaleDateString("en-US", { month: "long", day: "numeric" })}</span>
                <span className="w-1.5 h-1.5 rounded-full bg-[var(--border-medium)]" />
          ...
          <div className="flex flex-col items-center md:items-end w-full md:w-auto">
            <div className="text-[12px] font-[800] tracking-widest uppercase text-[var(--text-muted)] mb-1">OVERALL SCORE</div>
            <div className="flex items-baseline gap-2 mb-3">
              <span className="text-[72px] font-[800] leading-none tracking-tighter" style={{ color: getScoreColor(s.overall_score) }}>
                {s.overall_score?.toFixed(1) || "-"}
              </span>
              <span className="text-[24px] font-[700] text-[var(--text-muted)]">/5.0</span>
            </div>
        ...
        {/* AI SUMMARY */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-[20px] p-8 border-l-[4px]" style={{ background: 'var(--bg-tertiary)', borderLeftColor: 'var(--cue-green)' }}>
          <div className="text-[12px] font-[800] tracking-widest uppercase text-[var(--cue-green)] mb-3">Assessment Summary</div>
          <p className="text-[16px] leading-[1.7] text-[var(--text-secondary)] font-[500]">{s.summary}</p>
        </motion.div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {dimensions.map((dim, idx) => (
            <motion.div key={idx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 + (idx * 0.1) }}
              ...
              <div className="pl-4 py-1 italic text-[14px] leading-relaxed text-[var(--text-muted)] font-[500] border-l-[3px]" style={{ borderColor: dim.c }}>
              ...
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="w-full mt-8 overflow-hidden rounded-[20px] bg-white p-8 space-y-6" style={{ border: '1px solid var(--border-light)' }}>
                {data.messages?.map((msg: any, i: number) => (
                  <div key={i} className={`flex flex-col ${msg.role === 'candidate' ? 'items-end' : 'items-start'}`}>
                    ...
                    <div className={`px-6 py-4 text-[15px] leading-relaxed shadow-sm max-w-[85%] font-[500]`}
                      style={msg.role === 'ai' ? {
        ...
        <div className="flex justify-between items-center pt-8 border-t border-[var(--border-light)]">
          ...
          <button className="flex items-center gap-2 px-6 py-3 rounded-xl text-[14px] font-[800] border-2 border-[var(--cue-green)] text-[var(--cue-green)] hover:bg-[var(--cue-green)] hover:text-white transition-colors">
